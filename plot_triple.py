...
#        return -1
    ...
    # Load the input file.
    loaded = np.load('LCDM_JointMean.npz')
    ...
    loaded1 = np.load('w0wa_JointMean.npz')
    DH_hist1 = loaded1['DH_hist']
    DA_hist1 = loaded1['DA_hist']
    DH01 = loaded1['DH0']
    DA01 = loaded1['DA0']
    zvalues1 = loaded1['zvalues']
    lna1=-np.log(1 + zvalues1[::-1])
    fixed_options1 = loaded1['fixed_options']
    bin_range1 = loaded1['bin_range']
    hyper_range1 = loaded1['hyper_range']
    posterior_names1 = loaded1['posterior_names']

    loaded2 = np.load('w0wa2_JointMean.npz')
    DH_hist2 = loaded2['DH_hist']
    DA_hist2 = loaded2['DA_hist']
    DH02 = loaded2['DH0']
    DA02 = loaded2['DA0']
    zvalues2 = loaded2['zvalues']
    lna2=-np.log(1 + zvalues2[::-1])
    fixed_options2 = loaded2['fixed_options']
    bin_range2 = loaded2['bin_range']
    hyper_range2 = loaded2['hyper_range']
    posterior_names2 = loaded2['posterior_names']
    #if args.examples:
    ...
    #elif args.cosmo==1:
    cosmo1 = astropy.cosmology.Flatw0waCDM(H0=69.,Om0=0.3,w0=-0.9,wa=-0.75)
    #else:
    cosmo2 = astropy.cosmology.Flatw0waCDM(H0=69.,Om0=0.3,w0=-1.14,wa=0.35)
    ...
    for iperm,perm in enumerate(perms):
        ...
        #w0wa
        DH_ratio_limits1 = gphist.analysis.calculate_confidence_limits(
            DH_hist1[iperm],[args.level],bin_range1)
        DA_ratio_limits1 = gphist.analysis.calculate_confidence_limits(
            DA_hist1[iperm],[args.level],bin_range1)
        DH_ratio_limits_1sig1 = gphist.analysis.calculate_confidence_limits(
            DH_hist1[iperm],[0.68],bin_range1)
        DA_ratio_limits_1sig1 = gphist.analysis.calculate_confidence_limits(
            DA_hist[iperm],[0.68],bin_range1)

        # Convert to limits on DH, DA, with DA limits extended to z=0.
        DH_limits1 = DH_ratio_limits1*DH0
        DA_limits1 = np.empty_like(DH_limits1)
        DA_limits1[:,1:] = DA_ratio_limits1*DA0[1:]
        DA_limits1[:,0] = 0.

        #w0wa2
        DH_ratio_limits2 = gphist.analysis.calculate_confidence_limits(
            DH_hist2[iperm],[args.level],bin_range2)
        DA_ratio_limits2 = gphist.analysis.calculate_confidence_limits(
            DA_hist2[iperm],[args.level],bin_range2)
        DH_ratio_limits_1sig2 = gphist.analysis.calculate_confidence_limits(
            DH_hist2[iperm],[0.68],bin_range2)
        DA_ratio_limits_1sig2 = gphist.analysis.calculate_confidence_limits(
            DA_hist2[iperm],[0.68],bin_range2)

        # Convert to limits on DH, DA, with DA limits extended to z=0.
        DH_limits2 = DH_ratio_limits2*DH0
        DA_limits2 = np.empty_like(DH_limits2)
        DA_limits2[:,1:] = DA_ratio_limits2*DA0[1:]
        DA_limits2[:,0] = 0.
        ...
        H_true1 = cosmo1.H(zvalues).value
        DH_true1 = (const.c/cosmo1.H(zvalues)).to('Mpc').value
        DA_true1 = cosmo1.comoving_distance(zvalues).value

        H_true2 = cosmo2.H(zvalues).value
        DH_true2 = (const.c/cosmo2.H(zvalues)).to('Mpc').value
        DA_true2 = cosmo2.comoving_distance(zvalues).value
    ...
    # Plot evolution of DH/DH0, DA/DA0 over full redshift range.
        ...
        plt.plot(zvalues,DH_true1/DH0,'r')
        plt.plot(zvalues,DH_true2/DH0,'r')
        ...
        plt.fill_between(zvalues,DH_ratio_limits1[0],DH_ratio_limits1[-1],
            color=np.array((55,126,184))/255.,alpha=0.3)
        plt.fill_between(zvalues,DH_ratio_limits_1sig1[0],DH_ratio_limits_1sig1[-1],
            color=np.array((55,126,184))/255.,alpha=0.4)
        plt.fill_between(zvalues,DH_ratio_limits2[0],DH_ratio_limits2[-1],
            color=np.array((55,126,184))/255.,alpha=0.3)
        plt.fill_between(zvalues,DH_ratio_limits_1sig2[0],DH_ratio_limits_1sig2[-1],
            color=np.array((55,126,184))/255.,alpha=0.4)
        ...
        plt.plot(zvalues,DH_ratio_limits1[1],color=np.array((55,126,184))/255.,linewidth=4.0)
        plt.plot(zvalues,DH_ratio_limits_1sig1[1],color=np.array((55,126,184))/255.,linewidth=4.0)
        plt.plot(zvalues,DH_ratio_limits2[1],color=np.array((55,126,184))/255.,linewidth=4.0)
        plt.plot(zvalues,DH_ratio_limits_1sig2[1],color=np.array((55,126,184))/255.,linewidth=4.0)
        ...
        plt.plot(zvalues,DA_true1/DA0,'r')
        plt.plot(zvalues,DA_true2/DA0,'r')
        ...
        plt.fill_between(zvalues[1:],DA_ratio_limits_1sig[0],DA_ratio_limits_1sig[-1],
            color=np.array((55,126,184))/255.,alpha=0.4)
        plt.fill_between(zvalues[1:],DA_ratio_limits1[0],DA_ratio_limits1[-1],
            color=np.array((55,126,184))/255.,alpha=0.3)
        plt.fill_between(zvalues[1:],DA_ratio_limits_1sig1[0],DA_ratio_limits_1sig1[-1],
            color=np.array((55,126,184))/255.,alpha=0.4)
        plt.fill_between(zvalues[1:],DA_ratio_limits2[0],DA_ratio_limits2[-1],
            color=np.array((55,126,184))/255.,alpha=0.3)
        plt.fill_between(zvalues[1:],DA_ratio_limits_1sig2[0],DA_ratio_limits_1sig2[-1],
            color=np.array((55,126,184))/255.,alpha=0.4)
        #plt.plot(zvalues[1:],DA_ratio_limits[0],'b:')
        plt.plot(zvalues[1:],DA_ratio_limits[1],color=np.array((55,126,184))/255.,linewidth=4.0)
        plt.plot(zvalues[1:],DA_ratio_limits1[1],color=np.array((55,126,184))/255.,linewidth=4.0)
        plt.plot(zvalues[1:],DA_ratio_limits2[1],color=np.array((55,126,184))/255.,linewidth=4.0)
        ...
        DH_arr1 = DH_hist1[iperm,0,1:-1].reshape(100,10)
        sigma1 = 0.5*(DH_ratio_limits_1sig1[0,0]-DH_ratio_limits_1sig1[-1,0])
        mu1 = DH_ratio_limits_1sig1[1,0]
        bin21 = np.linspace(mu1-3*sigma1,mu1+3*sigma1,101,endpoint=True)

        DH_arr2 = DH_hist2[iperm,0,1:-1].reshape(100,10)
        sigma2 = 0.5*(DH_ratio_limits_1sig2[0,0]-DH_ratio_limits_1sig2[-1,0])
        mu2 = DH_ratio_limits_1sig2[1,0]
        bin22 = np.linspace(mu2-3*sigma2,mu2+3*sigma2,101,endpoint=True)
        ...
        plt.plot(const.c.to('km/s').value/(DH0[0]*bin21),np.exp(-(bin21-mu1)**2/(2*sigma1**2)),linewidth=4.0)
        plt.plot(const.c.to('km/s').value/(DH0[0]*bin22),np.exp(-(bin22-mu2)**2/(2*sigma2**2)),linewidth=4.0)
